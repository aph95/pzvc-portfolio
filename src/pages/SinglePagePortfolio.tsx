...
const SinglePagePortfolio = () => {
  ...
  const skills = ["UX Design", "Prototyping", "No-Code Development", "Front-End Experimentation", "Design Systems", "User Research"];
  const scrollToSection = (id: string) => {
    document.getElementById(id)?.scrollIntoView({
      behavior: 'smooth'
    });
  };
  return <div ref={containerRef} className="snap-container bg-background text-foreground relative">
      {/* HOME SECTION */}
      ...
      {/* ABOUT SECTION */}
      <section ref={aboutRef} id="about" className="min-h-screen relative snap-section flex items-center py-24 px-6">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            ...
            <div>
              ...
              delay: 0.4
                ...
                <a href="https://www.linkedin.com/in/aleksandar-hedstrom" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-3 bg-card border border-border/50 rounded-full text-sm hover:bg-accent hover:border-accent transition-all duration-300 group">
                  <Linkedin className="w-4 h-4 group-hover:text-purple-400 transition-colors" />
                  <span>LinkedIn</span>
                </a>
                <a href="mailto:alekansen@hotmail.com" className="flex items-center gap-2 px-5 py-3 bg-card border border-border/50 rounded-full text-sm hover:bg-accent hover:border-accent transition-all duration-300 group">
                  <Mail className="w-4 h-4 group-hover:text-purple-400 transition-colors" />
                  <span>Email</span>
                </a>
                <a href="tel:+46702574802" className="flex items-center gap-2 px-5 py-3 bg-card border border-border/50 rounded-full text-sm hover:bg-accent hover:border-accent transition-all duration-300 group">
      ...
      {/* PROJECTS SECTION */}
      <section ref={projectsRef} id="projects" className="min-h-screen relative snap-section py-24 px-6">
        {/* Background gradient */}
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
            ...
            delay: 0.3 + index * 0.15
          }} className={`group relative overflow-hidden rounded-3xl bg-card border border-border/30 hover:border-purple-500/30 transition-all duration-500 ${index === 0 ? 'md:row-span-2' : ''}`}>
                {/* Project image */}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.slice(0, 3).map(tag => <span key={tag} className="text-xs px-3 py-1 bg-purple-500/10 text-purple-300 rounded-full">
                        {tag}
                      </span>)}
                  </div>
                  ...
                  <div className="flex gap-4">
                    {project.prototypeUrl && <a href={project.prototypeUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-foreground hover:text-purple-400 transition-colors">
                        View Prototype
                        <ExternalLink className="w-4 h-4" />
                      </a>}
                    {project.videoUrl && <a href={project.videoUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-foreground hover:text-purple-400 transition-colors">
                        Watch Demo
                        <ExternalLink className="w-4 h-4" />
                      </a>}
                  </div>
      ...
      {/* CONTACT SECTION */}
      <section ref={contactRef} id="contact" className="min-h-screen relative snap-section flex items-center py-24 px-6">
        {/* Background gradient */}
        ...
        <div className="relative z-10 max-w-4xl mx-auto w-full text-center">
          ...
          delay: 0.3
            ...
            <a href="mailto:alekansen@hotmail.com" className="group p-6 sm:p-8 bg-card/50 border border-border/30 rounded-3xl hover:border-purple-500/30 hover:bg-card transition-all duration-300">
            ...
            <a href="https://www.linkedin.com/in/aleksandar-hedstrom" target="_blank" rel="noopener noreferrer" className="group p-6 sm:p-8 bg-card/50 border border-border/30 rounded-3xl hover:border-purple-500/30 hover:bg-card transition-all duration-300">
              <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
            ...
            <a href="tel:+46702574802" className="group p-6 sm:p-8 bg-card/50 border border-border/30 rounded-3xl hover:border-purple-500/30 hover:bg-card transition-all duration-300">
              <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 hidden md:flex flex-col gap-3">
        {['home', 'about', 'projects', 'contact'].map(section => <button key={section} onClick={() => scrollToSection(section)} className="w-3 h-3 rounded-full bg-white/20 hover:bg-white/60 transition-colors duration-300" aria-label={`Go to ${section}`} />)}
      </div>
    </div>;
};
